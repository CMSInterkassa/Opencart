...
class ControllerExtensionPaymentShoputilsIk extends Controller {
    ...
    const FILE_NAME_LOG = 'payment_shoputils_ik.log';
    ...
    public function index() {
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
            $this->_trimData(array(
                 'payment_shoputils_ik_shop_id',
                 'payment_shoputils_ik_sign_hash',
                 'payment_shoputils_ik_sign_test_key',
                 'payment_shoputils_ik_minimal_order',
                 'payment_shoputils_ik_maximal_order'
            ));
            ...
            $this->model_setting_setting->editSetting('payment_shoputils_ik', $this->request->post);
        ...
        $data = $this->_setData(array(
            ...
            'payment_shoputils_ik_success_url'  => $server . 'index.php?route=extension/payment/shoputils_ik/success',
            'payment_shoputils_ik_fail_url'     => $server . 'index.php?route=extension/payment/shoputils_ik/fail',
            'payment_shoputils_ik_pending_url'  => $server . 'index.php?route=extension/payment/shoputils_ik/success',
            'payment_shoputils_ik_status_url'   => $server . 'index.php?route=extension/payment/shoputils_ik/status',
            ...
            'log_lines'                 => $this->readLastLines(DIR_LOGS . 'payment_shoputils_ik.log', self::MAX_LAST_LOG_LINES),
        ...
        $data = array_merge($data, $this->_updateData(
            array(
                'payment_shoputils_ik_geo_zone_id',
                'payment_shoputils_ik_sort_order',
                'payment_shoputils_ik_status',
                'payment_shoputils_ik_minimal_order',
                'payment_shoputils_ik_maximal_order',
                'payment_shoputils_ik_order_status_id',
                'payment_shoputils_ik_order_fail_status_id',
                'payment_shoputils_ik_order_confirm_status_id',
                'payment_shoputils_ik_langdata',

                'payment_shoputils_ik_shop_id',
                'payment_shoputils_ik_sign_hash',
                'payment_shoputils_ik_sign_test_key',
                'payment_shoputils_ik_test_mode',
                'payment_shoputils_ik_currency',
                'payment_shoputils_ik_lifetime',
                'payment_shoputils_ik_log'
            ),
            array()
        ));
    ...
    protected function validate() {
        if (!$this->validatePermission()) {
            ...
            if (!isset($this->request->post['payment_shoputils_ik_shop_id']) || !trim($this->request->post['payment_shoputils_ik_shop_id'])) {
            ...
            if (!isset($this->request->post['payment_shoputils_ik_sign_hash']) || !trim($this->request->post['payment_shoputils_ik_sign_hash'])) {
                $this->error['warning'] = $this->error['error_sign_hash'] = sprintf($this->language->get('error_form'),
                                                                                        $this->language->get('entry_sign_hash'),
                                                                                        $this->language->get('tab_settings'));
            }

            if (!isset($this->request->post['payment_shoputils_ik_sign_test_key']) || !trim($this->request->post['payment_shoputils_ik_sign_test_key'])) {
    ...
    protected function makeUrl($route, $url = '') {
        return str_replace('&amp;', '&', $this->url->link($route, $url . '&user_token=' . $this->session->data['user_token'], true));
    }
